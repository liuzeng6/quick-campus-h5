<template>
		...
		<view class="bar-wrap">
			<block v-for="(item, index) in barList" :key="item.src">
				<view class="item" @click="checkedBar(index)">
					...
					<view v-if="!item.src.includes('fabu')"
						:class="checkedIndex == index ? 'item-text-active text' : 'text'">{{ item.text }}</view>
				</view>
			</block>
		</view>
	</view>
	<up-popup :show="show" mode="bottom" @close="show = false">
		<view class="box">
			...
			<view class="tag-list">
				<view v-for="item in tags" :key="item.id" @click="toRelease(item.id)">
